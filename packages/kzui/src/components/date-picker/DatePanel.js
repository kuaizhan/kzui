...
const getWeekIndex = (year, month, date) => {  //获取当前日期的周信息，信息模板为weekData
    const weekData = {
        year: year,
        week: 1,
        weekMonday: ''
    ...
    const firstMonTimeOfyear = firstDateOfyear.getTime() - (((firstDateOfyear.getDay() > 0 ? firstDateOfyear.getDay() : 7) - 1) * secondsOfOneDay * 1000)
    const thisWeekMondayTime = new Date(selectTime.getTime() - (((selectTime.getDay() > 0 ? selectTime.getDay() : 7) - 1) * secondsOfOneDay * 1000))

    weekData.week = parseInt((selectTime.getTime() - firstMonTimeOfyear) / (secondsOfOneDay * 7 * 1000)) + 1;
    weekData.weekMonday = `${thisWeekMondayTime.getFullYear()}-${thisWeekMondayTime.getMonth() + 1}-${thisWeekMondayTime.getDate()}`;

    if (month === 11 && date >= 25 ) {  //最后一个月的几天不满一周为下一年的第一周
...
class DatePanel extends KZUIComponent {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        const dataAno = nextProps.valueAno ? new Date(nextProps.valueAno) : todayDate;
    ...
    handleDateSelect(data) {
        ...
        this.setState({
            ...
            if (typeof (this.state.indexArr) === 'undefined' || this.state.indexArr === null) return;
            if (this.state.indexArr.length === 2 ||
                (this.state.indexArr.length === 1 && this.state.indexOther.length === 1)) {
    ...
    updateYear(year) {
        if (year >= this.state.minDate.getFullYear()
    ...
    render() {
        ...
        const { year, month, date, indexArr, hoverArr, monthAno, yearAno } = this.state;
        ...
        return (
            <div className={cls} style={style} onMouseOut={this.handleOut} tabindex="0" >
                <header>
                    <span className={`${clsPrefix}-left-controls`}>
                        <span onClick={this.handleYearDecrease}>
                            &lt;&lt;
                        </span>
                    ...
                    <span className={`${clsPrefix}-right-controls`}>
                        <span onClick={this.handleMonthIncrease}>
                            &gt;
                        </span>
                        <span onClick={this.handleYearIncrease}>
                            &gt;&gt;
                        </span>
                    </span>
                </header>
                <div className={`${clsPrefix}-title`}>
                    {nameOfDays.map((text, key) => (
                        <span className={`${clsPrefix}-day-title`} key={`title-${key}`}>{text}</span>
                    ))}
                </div>
                <div className={`${clsPrefix}-date-table`}>
                    {list.map((item, key) => (
                        <DateButton
                            ...
                            selected={indexArr !== null && typeof (indexArr) !== 'undefined' && indexArr.indexOf(item.index) !== -1 && item.className !== ''}
                            included={
                                hoverArr !== null && typeof (hoverArr) !== 'undefined' && item.index >= hoverArr[0] && item.index < hoverArr[1]
                            }
                        >
                            {item.text}
                        </DateButton>
