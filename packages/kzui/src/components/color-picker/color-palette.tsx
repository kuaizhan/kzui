...
interface ColorPaletteProps {
  ...
  hex?: string
  a?: number
  onChange?: (hex?: string, a?: number) => void
}

// 根据一个颜色获取画板右上角颜色
const getCornerColor = (rgb: number[]) => {
  const arr = [
    { key: 'r', val: rgb[0] },
    { key: 'g', val: rgb[1] },
    { key: 'b', val: rgb[2] }
...
> {
  isPressStrap: boolean
  paletteCtx: any
  strapCtx: CanvasRenderingContext2D
  paletteCanvas: HTMLCanvasElement
  strapCanvas: HTMLCanvasElement
  isPressPalette: boolean
  ...
  initStateFromProps (props: ColorPaletteProps) {
  ...
  getPosition (rgb) {
    ...
    switch (min + max) {
      case 'br':
        left = (cornerRgb[1] / 255) * 0.17
        break
      case 'bg':
        left = 0.34 - (cornerRgb[0] / 255) * 0.17
        break
      case 'rg':
        left = (cornerRgb[2] / 255) * 0.17 + 0.34
        break
      case 'rb':
        left = 0.68 - (cornerRgb[1] / 255) * 0.17
        break
      case 'gb':
        left = (cornerRgb[0] / 255) * 0.17 + 0.68
        break
      case 'gr':
        left = 1 - (cornerRgb[2] / 255) * 0.15
        break
      default:
        left = 0
        break
    ...
    const cursorLeft = `${parseInt(String(left * this.state.width), 10)}px`
    const pointTop = `${parseInt(
      String(1 - (arr[2].val / 255) * this.state.height),
      10
    )}px`
    const pointLeft = `${parseInt(
      String((1 - arr[0].val / arr[2].val) * this.state.width),
      10
    )}px`
    return {
      cursorLeft,
      ...
      <div className={cls}>
        <div className={typeClassName}>
          <div className={`${clsPrefix}-cpe-main`}>
            ...
            />
            <i className={`${clsPrefix}-cpe-point`} style={pointStyle} />
          </div>
          <div className={`${clsPrefix}-cpe-row`}>
            <i className={`${clsPrefix}-cpe-cursor`} style={cursorStyle} />
            <canvas
              width={this.state.width}
              height={this.state.strapHeight}
              ref={canvas => {
                this.strapCanvas = canvas
              }}
