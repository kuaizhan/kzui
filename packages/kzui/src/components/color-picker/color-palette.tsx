...
class ColorPalette extends KZUIComponent<
  ...
  getPosition(rgb) {
    ...
    const pointTop = `${parseInt(String((1 - (arr[2].val / 255)) * this.state.height), 10)}px`;
    const pointLeft = `${parseInt(String((1 - (arr[0].val / arr[2].val)) * this.state.width), 10)}px`;
    return {
      cursorLeft,
      pointTop,
      pointLeft,
  ...
  handleClickPalette(e, type) {
    // console.log(e.nativeEvent, 'click事件');
